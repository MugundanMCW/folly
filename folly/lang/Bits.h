...
#if defined(_MSC_VER) && defined(_M_ARM64) 
  // MSVC does not have __builtin_popcount, use __popcnt intrinsics.
  return static_cast<unsigned int>(
      sizeof(T) <= 4 ? 
        __popcnt(static_cast<unsigned int>(bits_to_unsigned<unsigned int>(v)))
        : __popcnt64(bits_to_unsigned<unsigned long long>(v))
      );
#else
  using U0 = unsigned int;
  using U1 = unsigned long int;
